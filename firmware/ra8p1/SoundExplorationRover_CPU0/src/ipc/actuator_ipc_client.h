...
IMPORT volatile UW g_cpu0_ipc_send_overflow_retry_count; /**< IPC送信overflow再試行回数（Live Watch用） */
IMPORT volatile UW g_cpu0_ipc_last_tx_message_id;        /**< 最終IPC送信メッセージID（Live Watch用） */
IMPORT volatile fsp_err_t g_cpu0_ipc_last_tx_error;      /**< 最終IPC送信ワードのエラー（Live Watch用） */
