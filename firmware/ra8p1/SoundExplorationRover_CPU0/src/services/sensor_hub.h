...
/* タスクへVL53L1Xドライバを公開せず、サービス層の診断値として線上の値を保持する。 */
typedef enum e_sensor_tof_result {
    SENSOR_TOF_RESULT_VALID = 0U,
    SENSOR_TOF_RESULT_RANGE_STATUS_INVALID,
    SENSOR_TOF_RESULT_DISTANCE_INVALID,
    SENSOR_TOF_RESULT_DATA_READY_TIMEOUT,
    SENSOR_TOF_RESULT_TRANSPORT_ERROR,
} sensor_tof_result_t;

#define SENSOR_TOF_RANGE_STATUS_UNAVAILABLE (0xFFU)
...
typedef struct st_sensor_diagnostics {
    ...
    sensor_failure_kind_t failure_kind;
    sensor_failure_device_t failure_device;
    sensor_failure_stage_t failure_stage;
    B failure_channel;
