...
extern volatile cpu0_audio_usb_state_t zg_cpu0_audio_usb_state;
extern volatile usb_status_t g_cpu0_audio_last_event;       /**< 最終USBイベント（Live Watch用） */
extern volatile uint8_t g_cpu0_audio_device_address;        /**< CDCアドレス（Live Watch用） */
extern volatile uint32_t g_cpu0_audio_event_count;          /**< USBイベント数（Live Watch用） */
extern volatile uint32_t g_cpu0_audio_transfer_busy_count;  /**< USB転送BUSY回数（Live Watch用） */
...
extern volatile uint32_t g_cpu0_audio_telemetry_send_count; /**< 診断送信完了数（Live Watch用） */
extern volatile uint32_t g_cpu0_audio_telemetry_busy_count; /**< 診断送信BUSY数（Live Watch用） */
/**< 最新音響観測（Live Watch用） */
extern volatile acoustic_observation_t g_cpu0_audio_observation;
