...
#include "../../cpu0_config.h"                              /* 距離、IMU、速度設定 */

LOCAL W obstacle_avoidance_abs_i16(H value);               /* Hの安全な絶対値 */
LOCAL BOOL obstacle_avoidance_snapshot_usable(const cpu0_sensor_snapshot_t * p_snapshot); /* 利用可能性判定 */
LOCAL BOOL obstacle_avoidance_imu_safe(const cpu0_sensor_snapshot_t * p_snapshot); /* IMU安全判定 */
LOCAL void obstacle_avoidance_output_stop(cpu0_sensor_rule_t rule, cpu0_think_state_t state,
                                          obstacle_avoidance_output_t * p_output); /* 停止指令生成 */
LOCAL void obstacle_avoidance_output_turn(cpu0_sensor_rule_t rule, H steering_deg, H outer_rpm, H inner_rpm,
 ...
 * ================================================================= */
LOCAL BOOL obstacle_avoidance_snapshot_usable(const cpu0_sensor_snapshot_t * p_snapshot) {
 ...
 * ================================================================= */
LOCAL BOOL obstacle_avoidance_imu_safe(const cpu0_sensor_snapshot_t * p_snapshot) {
 ...
 * ================================================================= */
EXPORT void obstacle_avoidance_controller_step(const cpu0_sensor_snapshot_t * p_snapshot, BOOL fault_active,
                                                obstacle_avoidance_output_t * p_output) {
    if (NULL == p_output) {
        return;
    }
