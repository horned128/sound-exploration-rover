...
static int16_t sound_follow_angle_normalize(int32_t angle_deg); /* 角度を-180～179度へ正規化 */
static int16_t sound_follow_relative_angle(uint16_t doa_deg); /* DoAを車体座標へ変換 */
static int16_t sound_follow_angle_delta(int16_t angle_deg, int16_t reference_deg); /* 円周上の符号付き角度差 */
static int16_t sound_follow_abs_i16(int16_t value);         /* int16_t絶対値 */
static bool sound_follow_observation_usable(const acoustic_observation_t * p_observation); /* 走行判断可能な観測判定 */
static bool sound_follow_observation_quiet(const acoustic_observation_t * p_observation); /* release条件判定 */
static void sound_follow_detection_reset(void);             /* 音量・DoA履歴初期化 */
static void sound_follow_doa_push(int16_t angle_deg);       /* DoA履歴追加 */
static bool sound_follow_doa_stable(int16_t * p_mean_deg);  /* DoA安定性と平均算出 */
static int16_t sound_follow_steering_from_doa(int16_t doa_deg); /* DoAから操舵角算出 */
static void sound_follow_motion_from_doa(int16_t doa_deg);  /* DoAから操舵・走行方向を決定 */
static void sound_follow_state_enter(cpu0_think_state_t state); /* 状態遷移 */
static void sound_follow_output_update(sound_follow_output_t * p_output); /* 状態から指令生成 */
...
static int16_t sound_follow_relative_angle(uint16_t doa_deg) {
    int16_t angle = sound_follow_angle_normalize((int32_t) doa_deg - CPU0_SOUND_DOA_ZERO_OFFSET_DEG);
 ...
 * @return -180～179度の差
 * ================================================================= */
static int16_t sound_follow_angle_delta(int16_t angle_deg, int16_t reference_deg) {
    return sound_follow_angle_normalize((int32_t) angle_deg - reference_deg);
}
 ...
 * ================================================================= */
static bool sound_follow_observation_usable(const acoustic_observation_t * p_observation) {
 ...
 * ================================================================= */
static bool sound_follow_observation_quiet(const acoustic_observation_t * p_observation) {
...
static void sound_follow_doa_push(int16_t angle_deg) {
    if (controller.doa_sample_count < CPU0_SOUND_DOA_SAMPLE_COUNT) {
        ...
        for (uint8_t index = 1U; index < CPU0_SOUND_DOA_SAMPLE_COUNT; index++) {
...
static bool sound_follow_doa_stable(int16_t * p_mean_deg) {
    ...
    for (uint8_t first = 0U; first < CPU0_SOUND_DOA_SAMPLE_COUNT; first++) {
        for (uint8_t second = (uint8_t) (first + 1U); second < CPU0_SOUND_DOA_SAMPLE_COUNT; second++) {
            int16_t const delta =
    ...
    *p_mean_deg = sound_follow_angle_normalize(sum / (int32_t) CPU0_SOUND_DOA_SAMPLE_COUNT);
    return true;
}
...
static void sound_follow_motion_from_doa(int16_t doa_deg) {
    ...
    if (reverse) {
        travel_angle_deg = sound_follow_angle_normalize((int32_t) doa_deg - 180);
    }

    int16_t steering_deg = sound_follow_steering_from_doa(travel_angle_deg);
    if (reverse) {
    ...
    int16_t left_rpm = (int16_t) (direction * CPU0_SOUND_MOVE_LEFT_RPM);
    int16_t right_rpm = (int16_t) (direction * CPU0_SOUND_MOVE_RIGHT_RPM);
    if (steering_deg > 0) {
        right_rpm = (int16_t) (direction * CPU0_SOUND_TURN_INNER_RPM);
    } else if (steering_deg < 0) {
        left_rpm = (int16_t) (direction * CPU0_SOUND_TURN_INNER_RPM);
    }
 ...
 * ================================================================= */
static void sound_follow_output_update(sound_follow_output_t * p_output) {
 ...
 * ================================================================= */
void sound_follow_controller_step(const sound_follow_input_t * p_input, uint32_t elapsed_ms,
