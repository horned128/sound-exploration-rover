/** =================================================================*
 * @file   sensor_i2c_bus.c
 * @brief  CPU0センサー用I2C1バス実装
 * ================================================================= */
#include "sensor_i2c_bus.h"                                 /* I2CバスAPI */
#include "../../cpu0_config.h"                              /* I2C有効化、転送timeout設定 */
...
LOCAL BOOL sensor_i2c_bus_open;                             /**< I2C1 open状態 */

LOCAL fsp_err_t sensor_i2c_bus_wait(i2c_master_event_t expected_event); /* 転送完了待ち */
LOCAL fsp_err_t sensor_i2c_bus_address_set(UB address);    /* 7bitスレーブアドレス設定 */

/** =================================================================*
 * @brief  FSP I2C転送完了コールバック
 * @param[in] p_args FSPから通知されるイベント
 * ================================================================= */
EXPORT void sensor_i2c_callback(i2c_master_callback_args_t * p_args) {
    if (NULL != p_args) {
 ...
 * @return FSPエラーコード
 * ================================================================= */
LOCAL fsp_err_t sensor_i2c_bus_wait(i2c_master_event_t expected_event) {
 ...
 * @return FSPエラーコード
 * ================================================================= */
LOCAL fsp_err_t sensor_i2c_bus_address_set(UB address) {
    if (!sensor_i2c_bus_open) {
 ...
 * @return FSPエラーコード
 * ================================================================= */
EXPORT fsp_err_t sensor_i2c_bus_init(void) {
    if (sensor_i2c_bus_open) {
        return FSP_SUCCESS;
    }
    ...
    err = R_IIC_MASTER_CallbackSet(&g_i2c_sensor_ctrl, sensor_i2c_callback, NULL, NULL);
    if (FSP_SUCCESS != err) {
 ...
 * @brief  I2C1をclose
 * ================================================================= */
EXPORT void sensor_i2c_bus_deinit(void) {
    if (sensor_i2c_bus_open) {
        (void) R_IIC_MASTER_Close(&g_i2c_sensor_ctrl);
        sensor_i2c_bus_open = FALSE;
    }
}
 ...
 * @return FSPエラーコード
 * ================================================================= */
EXPORT fsp_err_t sensor_i2c_bus_write(UB address, const UB * p_data, UW length) {
    if ((NULL == p_data) || (0U == length)) {
        return FSP_ERR_INVALID_ARGUMENT;
    }

    fsp_err_t err = sensor_i2c_bus_address_set(address);
    if (FSP_SUCCESS != err) {
        return err;
    }
    ...
    return (FSP_SUCCESS == err) ? sensor_i2c_bus_wait(I2C_MASTER_EVENT_TX_COMPLETE) : err;
}
 ...
 * @return FSPエラーコード
 * ================================================================= */
EXPORT fsp_err_t sensor_i2c_bus_read(UB address, UB * p_data, UW length) {
    if ((NULL == p_data) || (0U == length)) {
        return FSP_ERR_INVALID_ARGUMENT;
    }
    ...
    return (FSP_SUCCESS == err) ? sensor_i2c_bus_wait(I2C_MASTER_EVENT_RX_COMPLETE) : err;
}
 ...
 * @return FSPエラーコード
 * ================================================================= */
EXPORT fsp_err_t sensor_i2c_bus_write_read(UB address, const UB * p_write, UW write_length, UB * p_read,
                                           UW read_length) {
    ...
    err = sensor_i2c_bus_wait(I2C_MASTER_EVENT_TX_COMPLETE);
    if (FSP_SUCCESS != err) {
        return err;
    }
