...
LOCAL cpu0_sensor_diagnostics_t sensor_hub_diagnostics;     /**< 最終センサー診断 */
...
LOCAL cpu0_sensor_failure_device_t sensor_hub_tof_device(cpu0_tof_position_t position); /* ToF device取得 */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_select_stage(cpu0_tof_position_t position); /* ToF選択stage取得 */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_init_stage(cpu0_tof_position_t position); /* ToF初期化stage取得 */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_read_stage(cpu0_tof_position_t position); /* ToF読出しstage取得 */
LOCAL void sensor_hub_diagnostics_reset(cpu0_sensor_diagnostics_t * p_diagnostics); /* 診断初期化 */
LOCAL void sensor_hub_record_failure(cpu0_sensor_snapshot_t * p_snapshot, cpu0_sensor_failure_kind_t kind,
                                     cpu0_sensor_failure_device_t device, cpu0_sensor_failure_stage_t stage,
                                     B channel, fsp_err_t error); /* 最初の失敗を記録 */
LOCAL cpu0_sensor_failure_kind_t sensor_hub_transport_failure_kind(fsp_err_t error); /* 転送失敗種別取得 */
 ...
 * @return センサー診断device
 * ================================================================= */
LOCAL cpu0_sensor_failure_device_t sensor_hub_tof_device(cpu0_tof_position_t position) {
    switch (position) {
    case CPU0_TOF_LEFT:
 ...
 * @return センサー診断stage
 * ================================================================= */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_stage(cpu0_tof_position_t position,
                                                        cpu0_sensor_failure_stage_t left_stage) {
    return (cpu0_sensor_failure_stage_t) (left_stage + (UW) position * 2U);
}
 ...
 * @return センサー診断stage
 * ================================================================= */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_select_stage(cpu0_tof_position_t position) {
 ...
 * @return センサー診断stage
 * ================================================================= */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_init_stage(cpu0_tof_position_t position) {
 ...
 * @return センサー診断stage
 * ================================================================= */
LOCAL cpu0_sensor_failure_stage_t sensor_hub_tof_read_stage(cpu0_tof_position_t position) {
    return (cpu0_sensor_failure_stage_t) (CPU0_SENSOR_FAILURE_STAGE_TOF_LEFT_READ + (UW) position);
}
 ...
 * ================================================================= */
LOCAL void sensor_hub_diagnostics_reset(cpu0_sensor_diagnostics_t * p_diagnostics) {
    if (NULL == p_diagnostics) {
        return;
    }

    *p_diagnostics = (cpu0_sensor_diagnostics_t){
    ...
    for (UW index = 0U; index < CPU0_SENSOR_TOF_COUNT; index++) {
        p_diagnostics->tof_range_status[index] = VL53L1X_RANGE_STATUS_UNAVAILABLE;
        p_diagnostics->tof_result[index] = (UB) VL53L1X_RESULT_TRANSPORT_ERROR;
    }
}
 ...
 * @param[in] kind 失敗分類
 * @param[in] device 失敗device
 * @param[in] stage 失敗stage
 * @param[in] channel TCAチャネル
 * @param[in] error FSPエラー
 * ================================================================= */
LOCAL void sensor_hub_record_failure(cpu0_sensor_snapshot_t * p_snapshot, cpu0_sensor_failure_kind_t kind,
                                     cpu0_sensor_failure_device_t device, cpu0_sensor_failure_stage_t stage,
 ...
 * @return センサー失敗分類
 * ================================================================= */
LOCAL cpu0_sensor_failure_kind_t sensor_hub_transport_failure_kind(fsp_err_t error) {
 ...
 * ================================================================= */
EXPORT void sensor_hub_diagnostics_get(cpu0_sensor_diagnostics_t * p_diagnostics) {
    if (NULL != p_diagnostics) {
