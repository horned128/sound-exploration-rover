...
LOCAL fsp_err_t bmi270_write_register(UB register_address, UB value) {
    ...
    fsp_err_t const err = i2c_bus_write(bmi270_address, data, sizeof(data));
    if (FSP_SUCCESS == err) {
...
LOCAL fsp_err_t bmi270_write_registers(UB register_address, const UB * p_data, UW length) {
    ...
    fsp_err_t const err = i2c_bus_write(bmi270_address, packet, length + 1U);
    if (FSP_SUCCESS == err) {
        /* 設定モードのI2C書込み後に必要な450us以上のアイドル時間を確保する。 */
        R_BSP_SoftwareDelay(1U, BSP_DELAY_UNITS_MILLISECONDS);
    }
...
LOCAL fsp_err_t bmi270_read_registers(UB register_address, UB * p_data, UW length) {
    return i2c_bus_write_read(bmi270_address, &register_address, 1U, p_data, length);
}

/** =================================================================*
 * @brief  リトルエンディアン16 bit値の符号付き変換
