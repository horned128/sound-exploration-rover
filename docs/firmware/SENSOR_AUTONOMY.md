...
## 5. 方向を保持する回避と有限回の切り返し

通常走行ではToF距離から減速・操舵し、接近中は選んだ回避方向を保持します。
2026-09-15の実機ログでは、後退でCENTERが500 mmを超えるだけで旋回を終え、
左右ToFの数mmの差で逆方向へ操舵し直す周期動作が発生していました。
現在は**距離の確保、舵角整定、IMUでの回頭確認**を分けて判断します。

`PIVOT_LEFT / RIGHT`は既存のテレメトリ名です。現在の指令は外輪+120 RPM、
内輪+80 RPMの**前進を伴う円弧旋回**で、超信地旋回ではありません。
左右同符号の車輪を逆相4WSで操舵しても、その場回転にはならず車体前角の余裕が必要です。
後方距離センサーもないため、どんな配置でも脱出できることや後退時の無接触は保証できません。

| 条件・フェーズ | 動作 | 遷移・上限 |
|---|---|---|
| 通常走行 | 通常120 RPM、接近時100〜85 RPMを基準に距離比例で減速・操舵 | CENTER < 900 mmで回避方向を選択。初回だけ遠い側を選び、同距離なら左。以後の微小差で反転させない |
| 脱出開始 | CENTER ≤ 500 mm、または左右とも ≤ 380 mmかつCENTER ≤ 550 mm | BACKUPへ。150 mmの前端突出に加え、測距・減速の遅延分を見込む初期設定 |
| BACKUP準備 | 舵角0°、左右0 RPM、サーボ有効 | 400 ms整定してから後退する |
| BACKUP走行 | 舵角0°、左右-100 RPM | 後退600 ms以上、CENTER ≥ 750 mm、開始時から150 mm以上増加、左右とも ≥ 500 mmを満たして終了。後退上限2500 msでBLOCKED_STOP |
| PIVOT準備 | 舵角±45°、左右0 RPM、サーボ有効 | 400 ms整定してから旋回する |
| PIVOT走行 | 舵角±45°、外輪+120 RPM、内輪+80 RPM | 指定方向への累積回頭45°以上、旋回600 ms以上、CENTER ≥ 900 mm、旋回先 ≥ 500 mm、反対側 ≥ 380 mmの条件が300 ms継続して通常走行へ |
| 旋回中の再接近 | 正面の脱出条件または左右いずれか ≤ 380 mm | 方向と累積回頭量を維持して再度BACKUP。前角を監視しながら切り返す |
| 旋回進捗なし・時間切れ | 1200 msごとの正味回頭が5°未満、または旋回4500 ms | 回避方向を変更し、回頭量をリセットしてBACKUPから再試行 |
| 脱出後の方向保持 | 回避開始から正味100°に達するまでは、通常走行でも同じ向きへ最低30°操舵 | 前向き光軸が壁と平行に近づいて距離が伸びても、側面を壁へ寄せ続けない。1200 msごとの回頭進捗と通常操舵の10000 ms上限を監視 |
| 再試行上限 | 1回目を含めBACKUP開始は最大3回 | 次の再試行はBLOCKED_STOP。CENTER ≥ 1000 mm、左右とも ≥ 500 mmが1000 ms継続するか初期化するまで停止 |
| 極近接 | ToFのいずれか ≤ 150 mm | BLOCKED_STOP、駆動無効 |
| ToF/IMU異常・stale・傾斜・衝撃・CPU0 fault・学習中 | SAFE_STOP / IMU_STOP | 脱出状態も初期化。学習中は既存の非常停止指令を優先 |

通常走行中の回避方向と再試行履歴は、回避開始から100°以上回頭し、CENTER ≥ 1000 mm・左右とも ≥ 500 mmが1000 ms継続して初めて解除します。100°・最低30°は前向き3眼向けの保守的な初期設定で、小さな障害物に対しては遠回りになります。
